...
public class CameraFollow : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        if (isJumping)
        {
            // 점프 높이에 따라 줌 아웃 거리를 조정 (0~1 범위로 정규화)
            ...
            offset.z = Mathf.Clamp(originalOffset.z - dynamicZoomOut, -maxZoomOutDistance, originalOffset.z);
            ...
            // 점프 중일 때 부드러운 이동 속도 적용
            float currentSmoothSpeed = jumpSmoothSpeed;
            Vector3 smoothedPosition = Vector3.Lerp(transform.position, desiredPosition, currentSmoothSpeed);
            transform.position = smoothedPosition;
        }
        else
        {
            // 점프가 아닐 때 원래 위치로 돌아오도록 설정
            offset = originalOffset;
            Vector3 desiredPosition = target.position + offset;

            float currentSmoothSpeed = originalSmoothSpeed;
            Vector3 smoothedPosition = Vector3.Lerp(transform.position, desiredPosition, currentSmoothSpeed);
            transform.position = smoothedPosition;
        }
